...
class CanvasViewModel: ObservableObject {
    ...
    func applyUNetMask(_ result: UNetMaskResult) {
        guard let textureManager = renderer?.textureManager else {
        ...
        let maskWidth = Int(textureManager.maskSize.width)
        let maskHeight = Int(textureManager.maskSize.height)
        let srcWidth = Int(result.size.width)
        let srcHeight = Int(result.size.height)
        ...
        for y in 0..<maskHeight {
            for x in 0..<maskWidth {
                ...
                if result.isMultiClass {
                    // Multi-class: class ID directly from model output
                    newMaskData[dstIdx] = unetMask[srcIdx]
                } else {
                    // Binary: map 1 → currentClassID, 0 → background
        ...
        do {
            ...
            maskModified = true
            if result.isMultiClass {
                print("[UNet] Multi-class mask applied")
            } else {
    ...
    func applyServerPredictionMask(_ mask: [UInt8], sourceWidth: Int, sourceHeight: Int, isMultiClass: Bool = false) {
        let result = UNetMaskResult(
            mask: mask,
    ...
    /// Export current mask as 256×256 grayscale index PNG for server submission
    /// Pixel value = class ID (0=背景, 1-9=クラス)
