...
Copied from MRI_TOM project for compatibility with pretrained weights.
"""
...
class Up(nn.Module):
    ...
    def forward(self, x1, x2):
        x1 = self.up(x1)
        # 入力が2の冪乗サイズ(256等)かつbilinear=Trueの場合、
        # upsample後のサイズはskip connectionと常に一致するためpadは不要。
        # CoreMLはfloor_divide+int opを変換できないため削除。
